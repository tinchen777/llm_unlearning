...
class Plotter:
    def __init__(self, *run_dirs: Union[PathLike, str]):
        ...
        if len(self.runs) == 0:
            raise ValueError("No Experiment Loader provided.")
    ...
    def plot_training_curves(
        ...
        one line per run. X-axis is `"epoch"` (step is too fine-grained).
        """
        X_KEY = "epoch"
    ...
    def plot_tradeoff(
        ...
        for i, run in enumerate(self.runs):
            ...
            ax.annotate(
                run.label, (x, y), xytext=(6, 4), textcoords="offset points",

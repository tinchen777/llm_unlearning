...
# log_history keys that are diagnostics rather than losses
_TRAIN_SKIP_KEYS = {
    "learning_rate", "grad_norm", "train_runtime", "total_flos", "train_loss",
    "train_samples_per_second", "train_steps_per_second",
}


def _grid_axes(n_panels: int, ncols: int = 3, panel_size=(3.6, 2.6)):
...
def plot_training_curves(
    ...
    fig, axes = _grid_axes(len(keys), ncols)
    for ax, key in zip(axes, keys):
        n = 0
        for i, (run, hist) in enumerate(histories.items()):
            steps, values = collect_series(hist, key)
            if not steps:
                continue

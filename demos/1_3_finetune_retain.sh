...
python src/train.py --config-name=train.yaml \
  ...
  task_name=test/tofu_${MODEL1}_${retain_split} \
  # retain_logs_path=test/tofu_${MODEL1}_${retain_split}/TOFU_EVAL.json \
...
python src/train.py --config-name=train.yaml \
  ...
  forget_split=${forget_split} \
  holdout_split=${holdout_split} \
  task_name=test/tofu_${MODEL2}_${retain_split} \
  # retain_logs_path=test/tofu_${MODEL2}_${retain_split}/TOFU_EVAL.json \
  # trainer.args.per_device_train_batch_size=8 \
  # trainer.args.ddp_find_unused_parameters=true \
  # trainer.args.gradient_checkpointing=true \
  # --cfg job --resolve